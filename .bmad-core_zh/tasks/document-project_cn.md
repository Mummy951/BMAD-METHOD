...
为针对 AI 开发代理优化的现有项目生成全面的文档。此任务创建结构化参考材料，使 AI 代理能够理解项目上下文、约定和模式，从而有效地为任何代码库做出贡献。

## 任务说明

### 1. 初始项目分析

**关键：** 首先，检查上下文中是否存在 PRD 或需求文档。如果存在，则仅使用它将文档工作集中在相关区域。

**如果存在 PRD：**

- 审查 PRD 以了解计划中的增强功能/特性
- 识别将受影响的模块、服务或区域
- 仅将文档重点放在这些相关区域
- 跳过代码库中不相关的部分以保持文档简洁
...
“我注意到您没有提供 PRD 或需求文档。为了创建更具针对性和更有用的文档，我建议以下选项之一：

1. **首先创建 PRD** - 您希望我在文档化之前帮助创建老项目 PRD 吗？这有助于将文档重点放在相关区域。

2. **提供现有需求** - 您是否有可以共享的需求文档、Epic 或功能描述？

3. **描述重点** - 您能简要描述一下您计划的增强功能或特性吗？例如：
   - “为用户服务添加支付处理”
   - “重构身份验证模块”
   - “与新的第三方 API 集成”

4. **文档化一切** - 或者我应该继续对整个代码库进行全面文档化吗？（注意：这可能会为大型项目创建过多的文档）

请告诉我您的偏好，如果您愿意，我可以继续进行全面文档化。”

根据他们的回应：

- 如果他们选择选项 1-3：使用该上下文来重点关注文档
...
询问用户这些启发性问题，以更好地了解他们的需求：

- 这个项目的主要目的是什么？
- 代码库中是否有任何特定区域特别复杂或对代理商来说很重要？
- 您希望 AI 代理在此项目上执行什么类型的任务？（例如，错误修复、功能添加、重构、测试）
- 您是否偏好任何现有文档标准或格式？
- 文档应针对什么技术详细程度？（初级开发人员、高级开发人员、混合团队）
- 您是否计划了特定的功能或增强功能？（这有助于重点关注文档）
...
2. **提出澄清问题**：
   ...
   - “这个系统中开发人员最难理解的关键/复杂部分是什么？”
   - “我应该捕获任何未记录的‘部落知识’领域吗？”
   - “我应该记录哪些技术债务或已知问题？”
   - “代码库的哪些部分更改最频繁？”

3. **映射现实**：
   - 识别使用的“实际”模式（而非理论最佳实践）
   - 查找关键业务逻辑所在位置
   - 定位集成点和外部依赖项
   - 记录变通方法和技术债务
   - 注意与标准模式不同的区域

**如果提供了 PRD**：还要分析增强功能需要更改哪些内容

### 3. 核心文档生成

[[LLM：生成一份反映代码库“实际”状态的综合性老项目架构文档。

**关键**：这不是一份理想化的架构文档。文档化“存在”的内容，包括：
...
本文档捕获了 [项目名称] 代码库的“当前状态”，包括技术债务、变通方法和实际模式。它作为 AI 代理进行增强工作时的参考。

### 文档范围

[如果提供了 PRD：“重点关注与：{增强描述} 相关联的领域”]
...
- **相关类型**：TypeScript 定义在 `src/types/` 中
...
1. **支付服务**：`src/legacy/payment.js` 中的旧代码 - 紧密耦合，无测试
2. **用户服务**：与其他服务不同的模式，使用回调而不是 Promise
3. **数据库迁移**：手动跟踪，无适当迁移工具
4. **[其他重大债务]**

### 变通方法和注意事项

- **环境变量**：即使是暂存环境，也必须设置 `NODE_ENV=production`（历史原因）
...
- **前端通信**：端口 3000 上的 REST API，需要特定头部
...
3. **重点验证**：如果用户提供了范围，验证相关区域是否已突出显示
4. **清晰度评估**：检查解释对 AI 代理是否清晰
5. **导航**：确保文档具有清晰的部分结构，以便于参考

在主要部分之后应用高级启发任务，以根据用户反馈进行完善。

## 成功标准

- 创建了一个全面的老项目架构文档
- 文档反映了“现实”，包括技术债务和变通方法
- 关键文件和模块引用实际路径
- 模型/API 引用源文件而不是复制内容
...
- 此任务创建一份捕获系统“真实”状态的文档
- 尽可能引用实际文件而不是复制内容
- 诚实地记录技术债务、变通方法和约束
- 对于包含 PRD 的老项目：提供清晰的增强影响分析
- 目标是为 AI 代理进行实际工作提供“实用”文档